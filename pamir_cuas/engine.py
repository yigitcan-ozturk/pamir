...
def _fuse(observations: list[Observation]) -> float:
    """Simple deterministic prototype fusion score.

    This is deliberately not an operational C-UAS fusion algorithm. It only creates
    a repeatable decision surface for post-test forensic validation experiments.
    """
    if not observations:
        return 0.0
    signed = [o.confidence if o.supports_threat else -o.confidence for o in observations]
    score = 0.5 + (sum(signed) / (2 * len(observations)))
...
def analyze_incident(incident: dict[str, Any]) -> dict[str, Any]:
    ...
    threat_support = [o.confidence for o in observations if o.supports_threat]
    non_threat_support = [o.confidence for o in observations if not o.supports_threat]
    if threat_support and non_threat_support:
        if abs(max(threat_support) - max(non_threat_support)) <= DISAGREEMENT_THRESHOLD:
            anomalies.append(
                {
                    "type": "SENSOR_DISAGREEMENT",
                    "max_threat_confidence": max(threat_support),
                    "max_non_threat_confidence": max(non_threat_support),
                }
            )
    ...
    stale_ids = {
        a["observation_id"]
        for a in anomalies
        if a["type"] == "TEMPORAL_STALE_EVIDENCE"
    }
    for obs_id in sorted(stale_ids):
        ...
        replay = {
            ...
            "excluded_observation_id": obs_id,
            "score": replay_score,
            "decision": replay_decision,
            "changed_outcome": changed,
        }
        counterfactuals.append(replay)
        if changed:
            source = next(o.source_id for o in observations if o.observation_id == obs_id)
            causal_findings.append(
                {
                    "cause": "TEMPORAL_STALE_EVIDENCE",
                    "observation_id": obs_id,
                    "source_id": source,
                    "evidence": "removing the stale observation changes the fused decision",
                }
            )
    ...
    evidence_graph = {
        "nodes": [
            *[
            ...
            {"id": "fusion:baseline", "type": "FusionDecision", "score": baseline_score},
            {"id": "ground_truth", "type": "GroundTruth", "threat_present": truth.threat_present},
        ],
        "edges": [
            *[
                {
                    "from": o.observation_id,
                    "to": "fusion:baseline",
                    "type": "CONTRIBUTED_TO",
                }
                for o in observations
            ],
            {"from": "fusion:baseline", "to": "ground_truth", "type": "COMPARED_WITH"},
        ],
    }
    ...
    return {
        ...
        "baseline": {
            ...
            "ground_truth": truth.threat_present,
            "outcome": outcome,
        },
        "anomalies": anomalies,
        ...
        "pass": bool(causal_findings) and outcome in {"FALSE_POSITIVE", "MISSED_DETECTION"},
    }
