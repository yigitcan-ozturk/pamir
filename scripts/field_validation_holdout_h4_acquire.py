#!/usr/bin/env python3
"""Acquire blind-labelled Holdout H4 ULogs without running PAMIR."""
from __future__ import annotations
import hashlib,json,re,urllib.request,gzip
from pathlib import Path
DBINFO="https://review.px4.io/dbinfo"; REGISTER=Path("field-validation/HOLDOUT_H4_BLIND_REGISTER.md"); CORPUS=Path("field-validation/corpus/manifest.json"); OUT=Path("field-validation/runtime/holdout-h4-acquisition")
def sha256(p):
 h=hashlib.sha256()
 with p.open("rb") as f:
 ...
 return h.hexdigest()
def db():
 req=urllib.request.Request(DBINFO,headers={"User-Agent":"PAMIR-holdout-h4/1","Accept-Encoding":"gzip"})
...
def main():
 rows=re.findall(r"\| (H4-\d+) \| #(\d+) \| ([0-9a-f-]{36}) \| (healthy_control|incident|unknown) \|",REGISTER.read_text())
 ...
 for cid,issue,uid,label in rows:
  ...
  p=OUT/f"{uid}.ulg"
  try:
   req=urllib.request.Request(meta["download_url"],headers={"User-Agent":"PAMIR-holdout-h4/1"})
